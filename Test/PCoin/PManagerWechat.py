...
class PManagerWechat():
    def __init__(self):
        self.sqlHelper = SQLTool.sql_helper()

    def updatePCoin(self,nickName,changeValue,remark,time):
        puser,user=self.sqlHelper.getWeChatUser(nickName=nickName)
        self.sqlHelper.updatePCoin(user.id,changeValue=changeValue,remark=remark,time=time)

    def updatePValue(self,nickName,changeValue,remark,time):
        puser,user=self.sqlHelper.getWeChatUser(nickName=nickName)
        self.sqlHelper.updatePValue(user.id,changeValue=changeValue,remark=remark,time=time)

    def insertLog(self,nickName,action,remark,time):
        puser,user=self.sqlHelper.getWeChatUser(nickName=nickName)
        self.sqlHelper.insertPLog(user.id, action=action, remark=remark, time=time)

    def insertFood(self,uid,foodName,time,remark=""):
        self.sqlHelper.insertPiFood(uid=uid,foodName=foodName,time=time,remark=remark)

    def feedPiPi(self,nickName,foodName,time,remark=""):
        puser,user=self.sqlHelper.getWeChatUser(nickName=nickName)

        cost = FoodCalculator().getFoodValue(foodName)
        if user.p_coin >= cost:
            back = f"投食成功！ \n花费皮币{cost}个\n您还有皮币{user.p_coin - cost}个"
            self.updatePCoin(nickName=nickName, changeValue=-cost, remark="投食", time=time)
